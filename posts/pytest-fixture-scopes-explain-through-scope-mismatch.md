---
title: "pytest fixture scopes explained through scope mismatch"
slug: "downward-fixture-dependency-will-mess-up-your-teardown"
excerpt: "pytest fixture scopes explained through ScopeMismatch"
category: "testing"
date: "2026-05-27"
...
<br />
let's first remind ourselves that fixtures in pytest have a scope hierarch: <br />
...
```
you want an `admin_client` across the whole test session(<span className="font-medium"> may be its expensive to spin up!</span>) and you are tempted to think that, since you already have a `user` fixture why not hand it to this `admin_client` fixture. this will just mess up the teardown phase of pytest fixtures.<br /> `user` fixture will be torn down on every test function that request it but `admin_client` fixture is going to stay for the whole test session hence pytest will complain about fixture scope mismatch. instead, if a fixture depends on say a `function`-scoped fixture, it should itself be `function`-scoped and this applies going up too.<br /> <br />
in the above code, I would rather make a separate session scoped user fixture and leave function scoped user fixture for tests that need per-test isolation. 
```python
@pytest.fixture(scope="session")
def session_user(db):
    user = db.create(email="tmp@test.com", role="user")
...
<br />
ultimately, I think this hierarchy dependency makes sense and is kinda obvious too. I mean, would we have a module depending on an internal state of one of its callers? we can think of it in this way: <br /> *the more tests a fixture is shared across, the less it should know about any individual test and depending downward messes that up.*
